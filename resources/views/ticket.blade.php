    ...
    <div class="max-w-md w-full">
        ...
        <div class="bg-white text-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl relative">
            ...
            <div class="p-8 bg-indigo-50 border-b-4 border-dashed border-indigo-100 text-center relative">
                ...
                <h2 class="text-2xl font-black leading-tight">{{ optional($transaction->event)->title ?? 'Event Title' }}</h2>
            ...
            <div class="p-8 space-y-8">
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        ...
                        <p class="font-bold text-lg">{{ $transaction->customer_name ?? 'Nama Pembeli' }}</p>
                    </div>
                    <div>
                        <p class="text-slate-400 text-xs font-bold uppercase mb-1">Tanggal & Waktu</p>
                        <p class="font-bold text-lg">{{ optional($transaction->event)->date?->format('d M, H:i') ?? 'Tanggal & Waktu' }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="font-bold">{{ optional($transaction->event)->location ?? '-' }}</p>
                    </div>
                </div>
