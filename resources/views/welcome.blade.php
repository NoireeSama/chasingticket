...
<section id="events" class="max-w-7xl mx-auto px-6 py-20">
    ...
    <div class="mb-12 flex flex-wrap gap-3 justify-center items-center px-4 py-8 bg-gradient-to-r from-slate-50 to-indigo-50 rounded-2xl border border-slate-200">
        ...
        <a href="/" class="px-6 py-2.5 rounded-full font-semibold text-sm transition-all duration-300 transform hover:scale-105 {{ !request('category') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-white text-slate-700 border border-slate-300 hover:border-indigo-400 hover:bg-indigo-50' }}">
            ✓ Semua Kategori
        </a>

        @foreach($categories as $cat)
        <a href="/?category={{ $cat->slug }}" class="px-6 py-2.5 rounded-full font-semibold text-sm transition-all duration-300 transform hover:scale-105 {{ request('category') === $cat->slug ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-white text-slate-700 border border-slate-300 hover:border-indigo-400 hover:bg-indigo-50' }}">
            {{ $cat->name }}
        </a>
        @endforeach
    </div>
