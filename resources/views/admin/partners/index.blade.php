...
@section('page_subtitle', 'Atur mitra bisnis Anda di sini.')

@section('content')
<div class="mb-6 flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
    <form method="GET" action="{{ route('admin.partners.index') }}" class="flex flex-wrap md:flex-nowrap gap-3 items-center w-full md:w-auto flex-1">
        ...
        @if($search)
            <a href="{{ route('admin.partners.index') }}" class="px-6 py-3 bg-slate-200 text-slate-700 rounded-full font-bold text-sm hover:bg-slate-300 transition">
                Reset
            </a>
        @endif
    </form>

    <a href="{{ route('admin.partners.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-[#103370] text-white font-bold rounded-full shadow-[0_10px_20px_rgba(16,51,112,0.25)] hover:bg-[#F24781] hover:shadow-[0_10px_20px_rgba(242,71,129,0.35)] transition text-sm whitespace-nowrap self-stretch md:self-auto justify-center">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="px-8 py-6">
                        <div class="w-16 h-16 rounded-[20px] p-2 bg-slate-50 border-2 border-[#f1f5f9] shadow-sm flex items-center justify-center overflow-hidden">
                            <img src="{{ $partner->url_Logo }}" alt="{{ $partner->name }}" class="w-full h-full object-contain">
                        </div>
                    </td>

                    <td class="px-8 py-6">
                        <p class="font-extrabold text-[#103370] text-base">{{ $partner->name }}</p>
                    </td>

                    <td class="px-8 py-6">
                        <p class="text-xs text-slate-500 font-medium">{{ $partner->created_at->format('d M Y, H:i') }}</p>
                    </td>

                    <td class="px-8 py-6 text-center">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('admin.partners.edit', $partner->id) }}" class="p-2.5 bg-[#103370]/10 text-[#103370] rounded-full hover:bg-[#103370] hover:text-white transition shadow-sm" title="Edit Partner">
                            ...
                            <form action="{{ route('admin.partners.destroy', $partner->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus partner ini?');" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2.5 bg-rose-50 text-rose-600 rounded-full hover:bg-rose-600 hover:text-white transition shadow-sm" title="Hapus Partner">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-8 py-10 text-center text-slate-400 font-medium">Belum ada partner yang ditambahkan.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
