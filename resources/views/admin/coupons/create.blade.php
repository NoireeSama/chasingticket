...
@section('page_title', 'Tambah Kupon Baru')
@section('page_subtitle', 'Terbitkan kupon promosi baru untuk merchant Anda.')
...
<div class="bg-white border-2 border-[#f1f5f9] shadow-[0_20px_50px_rgba(16,51,112,0.06)] rounded-[35px] p-8 md:p-10 max-w-2xl relative overflow-hidden">
    ...
    <form action="{{ route('admin.coupons.store') }}" method="POST" class="space-y-6 relative z-10">
        @csrf

        <div>
            <label class="block text-xs font-black text-[#103370] mb-2 uppercase tracking-wider">Kode Kupon</label>
            <input type="text" name="code" id="code" placeholder="CONTOH: PROMO77" value="{{ old('code') }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-[#f1f5f9] rounded-full focus:ring-4 focus:ring-[#103370]/10 focus:border-[#103370] focus:bg-white outline-none transition font-black tracking-widest uppercase text-slate-800" required>
        ...
        <div>
            ...
            <input type="number" name="value" placeholder="Contoh: 10" min="1" max="100" value="{{ old('value') }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-[#f1f5f9] rounded-full focus:ring-4 focus:ring-[#103370]/10 focus:border-[#103370] focus:bg-white outline-none transition font-semibold text-slate-800" required>
        ...
        <div>
            ...
            <input type="date" name="expires_at" value="{{ old('expires_at') }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-[#f1f5f9] rounded-full focus:ring-4 focus:ring-[#103370]/10 focus:border-[#103370] focus:bg-white outline-none transition font-semibold text-slate-800">
        ...
        <div>
            ...
            <select name="is_limited" id="is_limited" class="w-full px-5 py-4 bg-slate-50 border-2 border-[#f1f5f9] rounded-full focus:ring-4 focus:ring-[#103370]/10 focus:border-[#103370] focus:bg-white outline-none transition font-bold text-slate-800 cursor-pointer" required>
                <option value="0" {{ old('is_limited') == '0' ? 'selected' : '' }}>Tidak Terbatas (Tanpa Kuota Penggunaan)</option>
                <option value="1" {{ old('is_limited') == '1' ? 'selected' : '' }}>Terbatas (Berdasarkan Maksimal Penggunaan)</option>
            </select>
            @error('is_limited') <span class="text-rose-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
        </div>

        <div id="limit_count_container" class="{{ old('is_limited') == '1' ? '' : 'hidden' }}">
            <label class="block text-xs font-black text-[#103370] mb-2 uppercase tracking-wider">Jumlah Kuota Kupon</label>
            <input type="number" name="limit_count" id="limit_count" placeholder="Masukkan jumlah maksimal penggunaan" value="{{ old('limit_count') }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-[#f1f5f9] rounded-full focus:ring-4 focus:ring-[#103370]/10 focus:border-[#103370] focus:bg-white outline-none transition font-semibold text-slate-800">
