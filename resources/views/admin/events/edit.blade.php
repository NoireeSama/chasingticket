...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
    ...
    <div class="lg:col-span-2 bg-white p-8 md:p-10 rounded-[35px] border-2 border-[#f1f5f9] shadow-[0_20px_50px_rgba(16,51,112,0.06)] relative overflow-hidden">
        ...
        <form action="{{ route('admin.events.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 relative z-10">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-xs font-black text-[#103370] mb-2 uppercase tracking-wider">Judul Event</label>
                <input type="text" name="title" id="titleInput" value="{{ old('title', $event->title) }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-[#f1f5f9] rounded-full focus:ring-4 focus:ring-[#103370]/10 focus:border-[#103370] focus:bg-white outline-none transition font-semibold text-slate-800" required>
            ...
            <div>
                ...
                <textarea name="description" rows="4" class="w-full px-5 py-4 bg-slate-50 border-2 border-[#f1f5f9] rounded-[25px] focus:ring-4 focus:ring-[#103370]/10 focus:border-[#103370] focus:bg-white outline-none transition font-semibold text-slate-800">{{ old('description', $event->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="datetime-local" name="date" value="{{ old('date', $event->date->format('Y-m-d\TH:i')) }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-[#f1f5f9] rounded-full focus:ring-4 focus:ring-[#103370]/10 focus:border-[#103370] focus:bg-white outline-none transition font-semibold text-slate-800" required>
                ...
                <div>
                    ...
                    <input type="text" name="location" value="{{ old('location', $event->location) }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-[#f1f5f9] rounded-full focus:ring-4 focus:ring-[#103370]/10 focus:border-[#103370] focus:bg-white outline-none transition font-semibold text-slate-800" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price', $event->price) }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-[#f1f5f9] rounded-full focus:ring-4 focus:ring-[#103370]/10 focus:border-[#103370] focus:bg-white outline-none transition font-semibold text-slate-800" required min="0">
                ...
                <div>
                    ...
                    <input type="number" name="stock" value="{{ old('stock', $event->stock) }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-[#f1f5f9] rounded-full focus:ring-4 focus:ring-[#103370]/10 focus:border-[#103370] focus:bg-white outline-none transition font-semibold text-slate-800" required min="1">
                    @error('stock') <span class="text-rose-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                </div>
            </div>

            <div>
                <label class="block text-xs font-black text-[#103370] mb-2 uppercase tracking-wider">Ganti Poster Event (Opsional)</label>
                ...
                <div class="relative border-2 border-dashed border-slate-200 hover:border-[#103370] bg-slate-50/70 hover:bg-white rounded-[30px] p-6 text-center transition-all cursor-pointer group" onclick="document.getElementById('posterInput').click()">
                    ...
                    <div id="uploadPlaceholder" class="space-y-2">
                        ...
                        <p class="font-bold text-slate-700 text-sm">Klik untuk memilih poster baru</p>
            ...
            <div class="p-6 bg-slate-50 rounded-[30px] border-2 border-[#f1f5f9] space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_dynamic_pricing" id="is_dynamic_pricing" value="1" class="sr-only peer" {{ old('is_dynamic_pricing', $event->is_dynamic_pricing) ? 'checked' : '' }}>
                        <div class="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#103370]"></div>
                    </label>
                </div>

                <div id="dynamic-pricing-container" class="{{ old('is_dynamic_pricing', $event->is_dynamic_pricing) ? '' : 'hidden' }} space-y-4 pt-3 border-t border-slate-200">
            ...
            <div class="pt-4 flex items-center justify-end gap-4 border-t border-slate-100">
                ...
                <button type="submit" class="px-8 py-4 bg-[#103370] text-white font-bold rounded-full text-sm shadow-[0_10px_25px_rgba(16,51,112,0.3)] hover:bg-[#F24781] hover:shadow-[0_10px_25px_rgba(242,71,129,0.35)] transition duration-300 transform active:scale-98">Simpan Perubahan</button>
            </div>
        </form>
    </div>
    ...
    <div class="lg:col-span-1 sticky top-6">
        <div class="bg-white p-6 rounded-[35px] border-2 border-[#f1f5f9] shadow-[0_15px_35px_rgba(0,0,0,0.04)] space-y-4">
            <div class="flex items-center justify-between">
                ...
                <span id="previewStatusBadge" class="px-3 py-1 bg-[#b8ff00] text-[#103370] rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm">
                    Poster Saat Ini
                </span>
            </div>

            @php
                $initialSrc = $event->poster_path ? asset('storage/' . $event->poster_path) : "data:image/svg+xml;charset=UTF-8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='400' height='500' viewBox='0 0 400 500'%3E%3Crect width='400' height='500' fill='%23f1f5f9'/%3E%3Ctext x='50%25' y='48%25' dominant-baseline='middle' text-anchor='middle' font-family='sans-serif' font-weight='bold' font-size='20' fill='%23103370'%3E" . urlencode($event->title) . "%3C/text%3E%3Ctext x='50%25' y='54%25' dominant-baseline='middle' text-anchor='middle' font-family='sans-serif' font-size='14' fill='%2394a3b8'%3ERasio 4:5%3C/text%3E%3C/svg%3E";
            @endphp

            <div class="relative w-full aspect-[4/5] rounded-[25px] overflow-hidden bg-slate-100 border-4 border-white shadow-md">
                <img id="posterPreview" src="{{ $initialSrc }}" data-default-src="{{ $initialSrc }}" alt="{{ $event->title }}" class="w-full h-full object-cover transition-all duration-300">
            </div>
    ...
    function previewPoster(event) {
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                if (badge) {
                    badge.innerText = 'Poster Baru Dipilih';
            ...
            if (badge) {
                badge.innerText = 'Poster Saat Ini';
                badge.className = 'px-3 py-1 bg-[#b8ff00] text-[#103370] rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm';
            }
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @if(old('dynamic_pricing_rules'))
            @foreach(old('dynamic_pricing_rules') as $rule)
                ...
                @endif
            @endforeach
        @elseif($event->dynamic_pricing_rules)
            @foreach($event->dynamic_pricing_rules as $rule)
                addRule('{{ \Carbon\Carbon::parse($rule['date'])->format('Y-m-d\TH:i') }}', '{{ $rule['price'] }}');
            @endforeach
        @endif
    });
</script>
